...
feature "xss" do
  ...
  scenario "attack\nTutorial: https://github.com/unboxed/security-hackathon/wiki/A3-Cross-Site-Scripting", js: true do
    login(normal_user)

    visit "/users/#{normal_user.id}/account_settings"
    within("#account_edit") do
      fill_in "First name", with: "<script>$(function() { $('div input.btn').val('Security Hackathon h4x0r3d') } )</script>"
    ...
    expect(find("#submit_button").value).not_to include("Security Hackathon h4x0r3d")
